...
#----------------------------------------------------------------------------
# cddlt diffusion module
# model always logs destd loss/metrics on dev to tb apart from other implemented models

class Diffusion(cddlt.DLModule):
    def __init__(
        self, 
        n_channels: int, 
        ...
        self.model = EDMPrecond(
            ...
            in_channels=n_channels + 1, # channels for target + noise and precondition 
    ...
    def train_step(self, x: torch.Tensor, y: torch.Tensor) -> None:
        ...
        rnd_normal = torch.randn([input.shape[0], 1, 1, 1], device=input.device)
        sigma = (rnd_normal * self.p_std + self.p_mean).exp()
        weight = (sigma ** 2 + self.sigma_data ** 2) / (sigma * self.sigma_data)**2
        target, augment_labels = (target, None)
        n = torch.randn_like(target) * sigma
        D_yn = self.model(target + n, sigma, input, augment_labels=augment_labels)
        self.optimizer.zero_grad()
        loss = weight * self.loss(D_yn, target)
        ...
        with torch.no_grad():
            ...
            self._update_metrics(D_yn, target)

    def eval_step(self, x: torch.Tensor, y: torch.Tensor) -> None:
        with torch.no_grad():
            input, coarse = x
            _, fine = y
            input, fine, coarse = input.to(self.device), fine.to(self.device), coarse.to(self.device)
            fine_hat = self.sample_model(input, coarse)
            loss = self.loss(fine_hat, fine)
            self._update_loss(loss)
            self._update_metrics(fine_hat, fine)

    def predict_step(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            input, coarse = x
            input, coarse = input.to(self.device), coarse.to(self.device)
            fine_hat = self.sample_model(input, coarse)
            return fine_hat
    ...
    def sample_model(
        ...
        cmp_sigma_min = max(self.sigma_min, self.model.sigma_min)
        cmp_sigma_max = min(self.sigma_max, self.model.sigma_max)

        init_noise = torch.randn((input.shape[0], 1, input.shape[2], input.shape[3]), dtype=torch.float64, device=self.device)

        # Time step discretization.
        step_indices = torch.arange(self.num_steps, dtype=torch.float64, device=init_noise.device)
        t_steps = (cmp_sigma_max ** (1 / self.rho) + step_indices / (self.num_steps - 1)
                * (cmp_sigma_min ** (1 / self.rho) - cmp_sigma_max ** (1 / self.rho))) ** self.rho
        t_steps = torch.cat([self.model.round_sigma(t_steps), torch.zeros_like(t_steps[:1])])  # t_N = 0
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
            ...
            # Euler step.
            denoised = self.model(x_hat, t_hat, input).to(
                torch.float64)
            ...
            if i < self.num_steps - 1:
                denoised = self.model(x_next, t_next, input).to(torch.float64)
...
# dataloader transform and collate
def residual_transform(sample: Dict[str, torch.Tensor]) -> Tuple[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor]]:
    input, target, coarse, fine = sample["input"], sample["target"], sample["coarse"], sample["fine"]
    return input, coarse, target, fine

def residual_collate_fn(batch: List[Tuple[Any]]) -> Tuple[Any, Any]:
    collate = []
    for idx in range(4): collate.append(torch.stack([item[idx] for item in batch]))
    return (collate[0], collate[1]), (collate[2], collate[3])